...
- `GET /api/progress` - Three momentum states with their evidence sentences and a 4-week trend, the consistency run in **weeks** with `graceUsed`, `recovery.medianDays`, `independence.ratio` (`null` until E12 records reminders), the ten most recent milestones plus every unacknowledged one, and the caller's confirmed memory insights
- `GET /api/progress/timeline?from=&to=&domain=&limit=&cursor=` - What actually happened, newest first. The mapping is a **whitelist**: pauses, continues, reschedules and fallback selections produce no event (PRD §76 asks for meaningful ones). Over 186 days is 400 `RANGE_TOO_LARGE`, never a silent truncation
- `GET /api/progress/milestones?unacknowledged=` - The six PRD §55 kinds. `sequence` distinguishes the n-th `FOUR_WEEKS`/`TEN_WORKOUTS` from the first; `REDUCED_REMINDERS` is dormant until E12 can measure independence
- `POST /api/progress/milestones/{id}/ack` - Celebrated once (PRD §77). Idempotent; 404 for a foreign id
...
The intervention ladder (PRD §25–§26, VISION §9). `GET /today` carries
`avoidance` on every **WORK** card and `null` on every other domain, and the
level drives `nextBestAction.interventionMode` — `DIAGNOSE` at 3–4,
`CHALLENGE_PLAN` at 5–6, `REDUCE` at 1–2:
- `GET /api/commitments/{id}/avoidance` - `{ level 0-6, interventionType, signals[], rationale, suggestedAction }`. **Derived on every read; there is no stored column** — the signals move overnight and a persisted level would contradict `GET /today` within hours
...
- `milestones` - Something worth marking, once (PRD §55). `sequence` is why the two repeatable kinds are not booleans — the fourth four-week stretch is a different fact from the first, while "ten workouts" said twice is the confetti PRD §77 rules out. The unique `(user_id, kind, sequence)` index IS the idempotency: the detector runs after every start, every completion, every comeback and once a day, and `createMany({skipDuplicates: true})` makes a re-award a no-op at the database rather than in a code path somebody could forget
