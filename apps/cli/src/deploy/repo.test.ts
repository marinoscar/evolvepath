...
function makeOrigin(defaultBranch = 'develop'): string {
  const dir = mkdtempSync(join(tmpdir(), 'evopath-origin-'));
...
function makeClone(origin: string, defaultBranch = 'develop'): string {
  const dir = mkdtempSync(join(tmpdir(), 'evopath-clone-'));
...
function deployRoot(): string {
  return mkdtempSync(join(tmpdir(), 'evopath-deploy-'));
}
...
describe('findGitRoot', () => {
  ...
  it('is undefined outside a checkout', () => {
    expect(findGitRoot(mkdtempSync(join(tmpdir(), 'evopath-nogit-')))).toBeUndefined();
  });
});
...
describe('resolveRepoTarget', () => {
  ...
  it('names --repo when there is no checkout to read', async () => {
    const outside = mkdtempSync(join(tmpdir(), 'evopath-nogit-'));
  ...
  it('names --repo when the checkout has no origin', async () => {
    const orphan = mkdtempSync(join(tmpdir(), 'evopath-orphan-'));
