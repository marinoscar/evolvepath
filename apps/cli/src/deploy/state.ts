...
//
// IT IS NOT IN ~/.evopath/config.json, AND THAT IS NOT A STYLE CHOICE.
...
// the next `evopath login` and then vanish, turning a working deployment into
...
export const DEPLOY_STATE_FILENAME = '.evopath-deploy.json';
...
export interface DeployState {
  ...
  /** Which evopath wrote this, for diagnosing a state file from the future. */
  evopathVersion: string;
...
export function readState(deployRoot: string): DeployState | undefined {
  ...
  if (version !== DEPLOY_STATE_VERSION) {
    ...
    // evopath having written it is the likeliest cause.
    throw new DeployStateError(
