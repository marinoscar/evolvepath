...
// common case for that. `evopath api POST /api/allowlist --data '{"email":...}'`
 ...
 *
 * THE TTY GUARD IS THE POINT OF THIS FUNCTION. Without it, `evopath api POST
  ...
  if (isTTY) {
    throw new UsageError(
      '--data - reads the request body from stdin, but stdin is a terminal. ' +
        'Pipe the body in (`cat body.json | evopath api ...`) or use --data @body.json.',
    );
  }
