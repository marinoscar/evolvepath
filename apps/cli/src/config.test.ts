...
// throwaway `$TMPDIR/evopath-config-test-XXXXXX` directory used as `home` via
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'evopath-config-test-'));
...
describe('requireCredentials — the three "nothing usable" messages', () => {
  it('names the missing variable when only EVOPATH_TOKEN is set (no file)', () => {
  ...
  it('names the missing variable when only EVOPATH_SERVER_URL is set (no file)', () => {
    const c: ConfigContext = {
      home: tmpDir,
