...
describe('evaluateTuiGate — stdin not a TTY while stdout is (case 3)', () => {
  // `echo | evopath` and `evopath < /dev/null`: stdout is a real terminal (the
...
describe('evaluateTuiGate — EVOPATH_NO_TUI, the explicit escape hatch (case 1)', () => {
  it('exports the env var name as EVOPATH_NO_TUI', () => {
    expect(NO_TUI_ENV_VAR).toBe('EVOPATH_NO_TUI');
  });
  ...
  it('EVOPATH_NO_TUI=false does not disable it', () => {
    const decision = evaluateTuiGate(
