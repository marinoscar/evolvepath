...
// process environment, never a real `~/.evopath` file) so `requireCredentials`
...
function baseCtx(
  ...
  return {
    ...
    // real home directory or a real `~/.evopath/config.json`.
    home: '/nonexistent-evopath-test-home-144',
    env: {
 ...
 * invoke `evopath api <args...>`. Mirrors config.test.ts's
