...
describe('evopath deploy doctor', () => {
  ...
  it('exits 6 when a required check fails', async () => {
    ...
    // "not ready" apart from "evopath itself broke".
...
describe('evopath deploy doctor --json', () => {
...
// ---------------------------------------------------------------------------
// `evopath deploy status`  (issue #183)
// ---------------------------------------------------------------------------

function installedRoot(): string {
  const root = mkdtempSync(join(tmpdir(), 'evopath-status-'));
...
describe('evopath deploy status', () => {
  ...
  it('distinguishes "nothing installed" from "installed and unhealthy"', async () => {
    const empty = mkdtempSync(join(tmpdir(), 'evopath-empty-'));
