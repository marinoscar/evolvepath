          ...
          <Routes>
            ...
            <Route element={<ProtectedRoute />}>
              ...
              <Route element={<RequireAiKey />}>
                ...
                {/* THE ONBOARDING GATE (#106, epic E04). Everything below it
                    needs a Path to be worth rendering: Today with no
                    commitments is an empty screen and Path with no Best Self is
                    an empty tree (PRD §101, §102).

                    THE ORDER OF THE THREE GATES IS LOAD-BEARING:

                      1. `ProtectedRoute` first, always — an anonymous visitor
                         never sees a setup page, they see login.
                      2. `RequireAiKey` before this one, because step 8 of the
                         wizard asks the coach for a plan. A user without a key
                         must fix that first, and removing a key mid-onboarding
                         correctly bounces to `/setup/ai-key` and returns via
                         `state.from`.
                      3. This gate last, around the shell only.

                    `/onboarding` sits INSIDE `RequireAiKey` and OUTSIDE this
                    gate — a gate that redirected its own destination would loop
                    forever. `OnboardingPage` itself sends a COMPLETED user to
                    `/`, so the exemption cannot be used to re-run onboarding.

                    `/settings/ai-key` stays inside BOTH gates: an un-onboarded
                    user manages their key at `/setup/ai-key`, which is the
                    exempt route.

                    ADMINS ARE GATED TOO — no role exemption. An admin who has
                    not onboarded is still a user of the product, and
                    `/admin/*` is as empty for them as Today is.

                    `NotificationProvider` stays INSIDE this gate, which is the
                    one-mount-point rule doing real work: a user still in the
                    wizard should not have an SSE stream open for a shell they
                    cannot reach. */}
