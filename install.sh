...
#   EVOPATH_BIN_DIR  Directory for the `evopath` shim (default: $HOME/.local/bin)
#   GITHUB_TOKEN     Optional GitHub PAT for private-repo clones
#   EVOPATH_SRC      Optional: local directory to install from (skips git clone).
#                    Useful for offline installs and local testing:
#                      EVOPATH_SRC=/path/to/repo bash install.sh
#
# NOTE: The public `curl | bash` flow requires the repository to be public (or
# GITHUB_TOKEN set for private repos). The EVOPATH_SRC path lets you verify
...
EVOPATH_SRC="${EVOPATH_SRC:-}"
GITHUB_TOKEN="${GITHUB_TOKEN:-}"

APP_DIR="$EVOPATH_HOME/app"
BIN_SHIM="$EVOPATH_BIN_DIR/evopath"
...
ENVIRONMENT VARIABLES
  EVOPATH_REPO      Git clone URL  (default: $EVOPATH_REPO)
  EVOPATH_REF       Branch/tag     (default: $EVOPATH_REF)
  EVOPATH_HOME      Install root   (default: \$HOME/.evopath)
  EVOPATH_BIN_DIR   Shim directory (default: \$HOME/.local/bin)
  GITHUB_TOKEN     GitHub PAT for private repos (optional)
  EVOPATH_SRC       Local source directory — skip git clone (optional)
                   Example: EVOPATH_SRC=/path/to/repo bash install.sh

NOTE
  The public curl | bash flow requires the repo to be public (or GITHUB_TOKEN
  set). Use EVOPATH_SRC for offline / local testing.
...
do_uninstall() {
  ...
  info "Config and credentials at $EVOPATH_HOME/config.json (if any) are left in place."
  ok "evopath CLI uninstalled."
}
...
printf '\n'
if _use_color; then
  printf '\033[36m  evopath CLI Installer\033[0m\n'
else
  printf '  evopath CLI Installer\n'
fi
printf '\n'
...
if [[ -d "$APP_DIR" ]]; then
  ...
  info "Installing evopath CLI to $APP_DIR"
fi
...
# Warn (don't fail) if the install target looks low on free space. evopath has
# no native modules, so the footprint is small — a few tens of MB for
# commander/ink/react and their transitive deps.
if command -v df &>/dev/null; then
  avail_kb="$(df -Pk "$EVOPATH_HOME" 2>/dev/null || df -Pk "$HOME" 2>/dev/null)"
...
if [[ -n "$EVOPATH_SRC" ]]; then
  if [[ ! -d "$EVOPATH_SRC" ]]; then
    err "EVOPATH_SRC directory not found: $EVOPATH_SRC"
    exit 1
  fi
  info "Using local source: $EVOPATH_SRC"
  # Copy to a temp dir so we don't pollute the working tree
  TMP_DIR="$(mktemp -d)"
  cp -r "$EVOPATH_SRC/." "$TMP_DIR/"
  ok "Copied source to temp dir"
else
  TMP_DIR="$(mktemp -d)"
  local_repo="$EVOPATH_REPO"

  # Inject GitHub token for private-repo support
  if [[ -n "$GITHUB_TOKEN" ]]; then
    # Replace https://github.com/ with https://<token>@github.com/
    local_repo="${EVOPATH_REPO/https:\/\/github.com\//https:\/\/$GITHUB_TOKEN@github.com\/}"
    info "Using GITHUB_TOKEN for authentication"
  fi

  info "Cloning $EVOPATH_REPO @ $EVOPATH_REF …"
  git clone --depth 1 --branch "$EVOPATH_REF" "$local_repo" "$TMP_DIR" 2>&1 \
    | grep -v "^$" | while IFS= read -r line; do dim "$line"; done || {
    err "Git clone failed. If the repo is private, set GITHUB_TOKEN or use EVOPATH_SRC."
    exit 1
  }
...
if [[ -n "$SRC_VERSION" && "$SRC_VERSION" != "unknown" ]]; then
  if [[ -z "$PREV_VERSION" || "$PREV_VERSION" == "unknown" ]]; then
    ok "Installing evopath CLI $(_c $BOLD "v$SRC_VERSION")"
  elif [[ "$PREV_VERSION" == "$SRC_VERSION" ]]; then
    ok "Reinstalling evopath CLI $(_c $BOLD "v$SRC_VERSION") (same version)"
  else
    ok "Updating evopath CLI $(_c $BOLD "v$PREV_VERSION") → $(_c $BOLD "v$SRC_VERSION")"
  fi
else
...
# ---------------------------------------------------------------------------
BIN_ON_PATH=0
if echo ":$PATH:" | grep -q ":$EVOPATH_BIN_DIR:"; then
  BIN_ON_PATH=1
fi
...
if [[ "$BIN_ON_PATH" != "1" ]] && ! is_wsl; then
  warn "$EVOPATH_BIN_DIR is not on your PATH"
  printf '\n'
  info "Add the following line to your shell config (~/.bashrc or ~/.zshrc):"
  printf '\n'
  printf '    %s\n' "export PATH=\"\$PATH:$EVOPATH_BIN_DIR\""
  printf '\n'
...
# freshly-installed `evopath` command is "not found" until the user appends it.
...
if is_wsl && [[ "$BIN_ON_PATH" != "1" ]]; then
  ...
  print_box "Windows 11 · WSL — one more step" \
    ...
    "" \
    "The 'evopath' command was installed to:" \
    "$EVOPATH_BIN_DIR" \
    ...
    "" \
    "echo 'export PATH=\"\$PATH:$EVOPATH_BIN_DIR\"' >> $RC_SHORT" \
    "source $RC_SHORT" \
    "" \
