...
> GitHub epic: [#60](https://github.com/marinoscar/evolvepath/issues/60)
...
- **What exists by E10 (dependency graph E01→…→E09→E10).** Domain state from E02: `Outcome`, `Plan`/`PlanVersion` (`DRAFT|ACTIVE|SUPERSEDED|REJECTED`, `createdBy USER|AI`, `previousVersionId`, `rationale`), `Routine` (belongs to a `PlanVersion`; `frequency DAILY|WEEKDAYS|WEEKENDS|WEEKLY|CUSTOM`, `daysOfWeek Int[]`, `preferredTime 'HH:mm'`, `estimatedDurationMin`, `minimumDurationMin`, `fallbackBehavior`, `active`), `Commitment` (nine-state `CommitmentStatus`, `rescheduleCount` carried onto the new row by E02-04 (#47)'s reschedule, `routineId`, `planVersionId`, `scheduledStart`), `Evidence` (`source USER_LOG|TIMER|WORKOUT_LOG|APP_FLOW`), `Reflection` (`relatedType 'day'`, `frictionTags String[]`, E05-03 (#43)), `DomainMode` (`GROW|MAINTAIN|RECOVER|PAUSE`, `PUT /api/me/domain-modes/:domain`, `DomainModesService.set`, E02-02 (#39)). E05-02 (#40) adds `Commitment.versionUsed FULL|SHORT|MINIMUM`, `startedAt`, `minutesSpent` and the `fallback` action — the aggregation reads those columns. E07-02 (#110) adds `focus_sessions`, E09 adds `workout_sessions`; E07-05 (#120) (`GET /work/summary`) and E08-03 (#45) (`GET /family/summary`) compute per-domain weekly numbers for their own screens — E10 does **not** call those endpoints, it reads the same tables in one pure function so the three domains are aggregated identically (PRD §14.6 "domain balance").
- **Profile.** `user_profiles` (E04-01 (#100)): `timezone`, `coachingStyle GENTLE|BALANCED|DIRECT`, `weekdayMinutes`, `onboardingCompletedAt`; `UserProfileService.getOrCreate/update`. E04 shipped no general `PATCH /me/profile`; the review day/time settings this epic adds get their own endpoints in the weekly module.
- **AI.** `AiGatewayService.invoke({persona, userId, promptVersion, instructions, input, schema, schemaName})` (E01-06 (#26), `apps/api/src/ai/gateway/ai-gateway.service.ts`) returns `{ok:true, invocationId, output}` or `{ok:false, invocationId, error:{code,message}}` and never throws for provider problems. `weekly_reviewer` is already in `apps/api/src/ai/ai-personas.ts` (E01-02 (#22), tier `reasoning`). Context comes from `ContextAssemblerService.assemble(userId, 'planner')` + `renderForPrompt` (E06-02 (#63)); prompts are versioned constants (`apps/api/src/coach/prompts/coach.prompt.ts` is the model). Chain of thought is never stored (PRD §88).
- **Mutation protocol.** `ProposalsService.createFromSource(userId, sourceKind, {planId, summary, changes, invocationId})` (E06-04 (#76), `apps/api/src/coach/proposals/proposals.service.ts`) with `sourceKind: 'WEEKLY_REVIEW'` (enum member already declared in E06-01 (#61)'s `ProposalSourceKind`), `planChangeSchema` (`apps/api/src/coach/proposals/plan-change.schema.ts`), and `POST /api/proposals/:id/accept|edit|reject`. The web `PlanChangeDiff` and `ProposalCard` components (`apps/web/src/components/coach/`, E06-07 (#86)) are exported for this epic. `PatternAnalysisService.proposeInsights(userId)` (E06-05 (#78)) is the hook E10 calls after each review.
...
- **Local time.** `apps/api/src/today/local-date.ts` (E05-01 (#38)) has `localDate(now, timeZone)` and `localDayBounds` built on `Intl.DateTimeFormat`, no date library. Weeks in this epic are **Monday-start, in the user's timezone**, and are addressed by the local Monday as a `'YYYY-MM-DD'` string (the same representation E05-03 (#43) chose for `daily_check_ins.dateLocal`).
- **Notifications.** `NotificationsService.notify(eventKey, userId, data)` is detached and never throws; `findEvent(key)` (`apps/api/src/notifications/notification-events.ts`) answers whether an event is registered. E12-02 (#54) registers the coaching events including `coach.weekly_review_ready` (PRD §60 N8 "Your week is ready to review"); E10 calls `notify` only when `findEvent` finds it.
- **Web shell.** `DESTINATIONS` (E02-05 (#51), `apps/web/src/config/destinations.ts`): `progress` owns `/progress` (placeholder `ProgressPage`, E11 makes it real). Every route under `/progress/*` is already owned by `progress`, so this epic adds routes without touching `DESTINATION_ROUTES`. Settings cards live in `apps/web/src/config/userSettingsSections.tsx` (`USER_SETTINGS_SECTIONS`) and render through `SettingsHub`; the compact AppBar drill-down table is `resolveDrillDown` in `apps/web/src/components/navigation/AppBar.tsx` (E02-06 (#56) added the first non-settings entry). The five coupled breakpoint gates (CLAUDE.md rule 5) are not touched.
- **Patterns to copy.** Ownership-checked per-user resources: `apps/api/src/pat/` (plain `@Auth()`, own rows only; `apps/api/src/path/owned-resource.ts` from E02-02 (#39)). Audit: direct `prisma.auditEvent.create` as in `apps/api/src/email/email-settings.service.ts`. Zod DTOs via `createZodDto` (`apps/api/src/email/dto/update-email-settings.dto.ts`). OpenAPI tags in `apps/api/src/openapi/tags.ts` (`EvolvePath` group created by E02). Integration harness `apps/api/test/helpers/test-app.helper.ts` (`createTestApp({ overrideProviders })`). Fake OpenAI server `tools/fake-openai/server.mjs` + `scenarios/` (E01-10 (#30), E06-09 (#93)).
- Design rationale and rejected alternatives are written by E10-05 (#89) in `docs/specs/weekly-review.md` (new).

### Scope

- [ ] #65 `feat(db): add weekly reviews, weekly plans and review-rhythm profile fields` (E10-01)
- [ ] #73 `feat(api): add weekly review generation with deterministic aggregation, reviewer persona and scheduled runs` (E10-02)
- [ ] #80 `feat(api): add weekly planning flow with constraints, domain modes, load check and approve` (E10-03)
- [ ] #84 `feat(web): add Weekly Review screen, Weekly Planning wizard and Weekly rhythm settings` (E10-04)
- [ ] #89 `test(tests): E10 end-to-end verification` (E10-05)
...
- The `coach.weekly_review_ready` event definition, browser/push copy and quiet-hour policy (E12-02 (#54)/E12-03 (#59)). E10 only calls `notify()` when the event exists.
- Automatic `MISSED` marking of stale commitments (E11-02 (#112) comeback loop). Until then, past `PLANNED`/`READY` rows are aggregated as `unresolved`, not `missed`.
- Calendar import of fixed events (PRD §69, later V1/V2); constraints are typed by the user.
- AI-worded commitment titles in weekly planning (the `planner` persona). Materialisation is deterministic from routines; wording help is a P1 follow-up noted in E10-03 (#80).
- Editing routines or plan versions from the review screen beyond accepting/editing/rejecting proposals (Path, E02-06 (#56), owns that).
- Multi-week look-back trends, exports, sharing (PRD §78, §79 later).

### Sequencing

- **Critical path:** E10-01 (#65) → E10-02 (#73) → E10-03 (#80) → E10-04 (#84) → E10-05 (#89).
- E10-02 (#73) and E10-03 (#80) both extend `apps/api/src/weekly/` (`WeeklyModule`, `tags.ts`, `docs/API.md`); run them sequentially to avoid merge conflicts. E10-03 (#80)'s pure `materialize-week.ts` and `load-check.ts` can be written before E10-02 (#73) lands, but its module wiring waits.
- E10-04 (#84) needs both API children (the screen renders a review; the wizard drives a plan). Its settings page needs only E10-02 (#73)'s `/weekly/settings`.
- E10-05 (#89) is last and additionally depends on E01-10 (#30) (fake server) and E06-09 (#93)'s scenario matcher.
...
3. http://localhost:3535/testing/login → `week-owner@test.local`, role `contributor`, AI-key checkbox ticked (E01-10 (#30)), "Mark onboarding complete" ticked (E04-06 (#107)). You land on `/`.
...
model WeeklyReview {
  ...
  proposalIds   String[]           @default([]) @map("proposal_ids")    // PlanChangeProposal ids; no FK (E06-04 (#76) expires/prunes)
...
model WeeklyPlan {
  ...
  proposal            Json?                                             // weeklyPlanProposalSchema (E10-03 (#80))
...
`User` gains `weeklyReviews WeeklyReview[] @relation("UserWeeklyReviews")` and `weeklyPlans WeeklyPlan[] @relation("UserWeeklyPlans")`. `UserProfile` (E04-01 (#100)) gains:
...
export const domainEnum = z.enum(['WORK','FAMILY','HEALTH']);            // mirrors Prisma `Domain` (E02-01 (#36))
...
export const weeklyReviewOutputSchema = z.object({
  ...
  proposedChanges: z.array(z.object({ planId: z.string().uuid(), summary: z.string().min(1).max(300), changes: z.array(planChangeSchema).min(1).max(10) })).max(3), // planChangeSchema from E06-04 (#76)
...
Export inferred types (`WeekAggregates`, `WeeklyReviewOutput`, `WeeklyReviewSummary`, `WeeklyPlanConstraints`, `WeeklyDomainModes`). `weeklyPlanProposalSchema` is declared by E10-03 (#80) in the same file.

**API (backend-dev)** — no endpoints. Add the two Prisma delegates to `apps/api/test/mocks/prisma.mock.ts` (`weeklyReview`, `weeklyPlan`) and extend `UserProfileService.update` (E04-01 (#100)) so the typed partial accepts `weeklyReviewWeekday` and `weeklyReviewTime`. Run `npm run prisma:generate` and `npm run typecheck`.
...
- Services, endpoints, the cron task (E10-02 (#73)); materialisation and load check (E10-03 (#80)).
...
- `weekStart` is a `'YYYY-MM-DD'` **string**, not `@db.Date`: a `Date` column round-trips through Prisma as UTC midnight and shifts by a day for users west of Greenwich; E05-03 (#43) made the same call for `daily_check_ins.date_local`. Validate it with `isoDate` at every boundary.
- `proposalIds` is deliberately a string array without a FK: E06-04 (#76) marks proposals `EXPIRED` lazily and they may be pruned; the review must survive that. Write a short comment above the model in the style of the `PlanChangeProposal` comment (E06-01 (#61)).
- `weeklyReviewOutputSchema` imports `planChangeSchema` from `apps/api/src/coach/proposals/plan-change.schema.ts` (E06-04 (#76)) — do not redeclare the change shape.
...
- Self-explanatory `@map` snake_case on every column, `@@map` plural tables, `@db.Timestamptz` on every `DateTime` — the E02-01 (#36) conventions.

---

### E10-02 `feat(api): add weekly review generation with deterministic aggregation, reviewer persona and scheduled runs` — #73

**Part of epic:** E10 · **Blocked by:** E10-01 (#65), E06-02 (#63), E06-04 (#76), E06-05 (#78), E05-02 (#40), E05-03 (#43) · **Component:** api · **Priority:** P0 · **Agents:** backend-dev → testing-dev → docs-dev
...
A new `apps/api/src/weekly/` module: a pure aggregation function, a `weekly_reviewer` invocation with a strict six-output contract whose `proposedChanges` become `PlanChangeProposal`s through E06-04 (#76), a template fallback, an hourly sweep that generates reviews at each user's local review time, on-demand generation, and the review-rhythm settings endpoints.

**Data (database-dev)** — n/a (E10-01 (#65)).
...
- `apps/api/src/weekly/weekly-review.service.ts` — `WeeklyReviewService`:
  ...
  - `markApproved(userId, weekStart, tx)` — used by E10-03 (#80)'s approve (`READY → APPROVED`, `approvedAt`; no-op when absent or already `APPROVED`).
...
3. Epic step 13 (AI down → template) and step 11 (409 after approve, once E10-03 (#80) exists — until then skip).
...
- Weekly planning, materialisation, load check, `markApproved` callers (E10-03 (#80)).
- Notification event definition/copy (E12-02 (#54)), push delivery (E12-04 (#64)).
- Multi-week trend inputs to the reviewer (E11 momentum; PRD §14.6 "notification interactions" input arrives with E12-06 (#69) — pass `recentNotificationCount` from the context until then).

#### Notes for the implementing agent

- Reuse, do not re-derive: `localDate`/`localDayBounds` (`apps/api/src/today/local-date.ts`), `ContextAssemblerService.assemble/renderForPrompt` (E06-02 (#63)), `ProposalsService.createFromSource` (E06-04 (#76) — never `PlansService.createVersion` from here), `findEvent` (`notification-events.ts`), the throttle in `apps/api/src/ai/gateway/test-throttle.ts`, the `@Cron` shape in `apps/api/src/auth/tasks/token-cleanup.task.ts`.
- `aggregateWeek` must stay pure and side-effect free; it is unit-tested with fixtures and reused by E10-03 (#80)'s load summary and E11's momentum inputs. Put Prisma reads in `AggregationService.load` only.
...
### E10-03 `feat(api): add weekly planning flow with constraints, domain modes, load check and approve` — #80

**Part of epic:** E10 · **Blocked by:** E10-02 (#73), E02-02 (#39), E02-03 (#42), E02-04 (#47) · **Component:** api · **Priority:** P0 · **Agents:** backend-dev → testing-dev → docs-dev
...
- The wizard UI (E10-04 (#84)); AI wording of commitment titles via the `planner` persona (P1: a `?wording=ai` flag on `/propose` would call `planner` with the item list and return titles only — deterministic materialisation stays the source of ids and times).
- Ritual recurrence for FAMILY (E08-02 (#41) owns rituals; if E08 routines are modelled as `Routine` rows they are materialised here like any other).
- Rebalancing suggestions ("replace X with Y") — the warning only recommends replacing; the user chooses.

#### Notes for the implementing agent

- `materializeWeek` and `checkLoad` are pure and must not import Prisma or Nest; keep them reusable by E08-02 (#41) and by the web (the wizard shows the same summary the API computed — it does not recompute).
- Local date+time → UTC: build the instant with `localDayBounds(date, tz).start` + minutes (E05-01 (#38) helper); never `new Date('YYYY-MM-DDTHH:mm')` (that is parsed in the server's zone).
- Commitments go through `CommitmentsService.create` (E02-04 (#47)) so audit `commitment:create`, ownership checks and the `planVersionId` ACTIVE/DRAFT rule apply; pass the transaction client.
- Use `DomainModesService.set` (E02-02 (#39)) so `domain_mode:set` audit rows are written with `reason`; do not upsert `domain_modes` directly.
...
- `ReviewProposalCard.tsx` `{ proposal, onDecide }` — wraps E06-07 (#86)'s `ProposalCard` (renders `PlanChangeDiff`; Accept / Edit / Keep current plan); after accept shows "Plan updated (v<N>)" with a link to `/path/outcomes/<outcomeId>`.
...
Responsive: below `sm` (600px) everything stacks, the stepper is vertical, `BottomNav` stays visible (the wizard is inside `Layout`, not full-screen — unlike E09-08 (#109)'s runner; a weekly plan is not a live activity); ≥`sm` tiles in a row and horizontal stepper; ≥`md` two-column review. The five coupled gates are untouched.
...
`data-testid`s for E10-05 (#89): `week-tile-WORK|FAMILY|HEALTH`, `review-generate`, `review-pattern`, `review-proposal`, `review-approve-next-week`, `wizard-next`, `wizard-add-commitment`, `wizard-load-warning`, `wizard-approve`, `wizard-ack-warnings`, `rhythm-weekday`, `rhythm-time`, `rhythm-save`.
...
**Docs (docs-dev)** — `CLAUDE.md` "Access Control"/"Settings UI" examples unchanged; add `/settings/weekly-rhythm` to the user-settings card list wherever `docs/specs/settings-ui.md` (E01-11 (#31)) enumerates cards; `docs/specs/weekly-review.md` section "Screens" (routes, states, responsive rules).
...
- The Progress screen proper (momentum cards, timeline — E11-04 (#117)); only the `WeekEntryCard` is added to the placeholder.
- Editing routines/plan versions inline (Path, E02-06 (#56)); editing proposals beyond E06-07 (#86)'s `EditProposalDialog`.
- A timezone editor (belongs to Profile; E04 owns `timezone`).
- Push/browser notification for "your week is ready" (E12).

#### Notes for the implementing agent

- Import `ProposalCard` and `PlanChangeDiff` from `apps/web/src/components/coach/` (E06-07 (#86)); do not fork the diff rendering. If `ProposalCard` is coupled to the chat message shape, extract a `ProposalCardBody` in E06's folder and use it from both places (a small, related refactor — commit it separately as `refactor(web): …`).
...
- The wizard's `useMediaQuery(down('sm'))` for stepper orientation is a local layout choice; write the comment E01-07 (#27)'s `PersonaModelTable` uses ("not one of the five coupled gates").
...
### E10-05 `test(tests): E10 end-to-end verification` — #89

**Part of epic:** E10 · **Blocked by:** E10-01 (#65), E10-02 (#73), E10-03 (#80), E10-04 (#84), E01-10 (#30), E06-09 (#93) · **Component:** tests, docs · **Priority:** P0 · **Agents:** testing-dev → ops-dev → docs-dev
...
**Fake server (testing-dev)** — extend `tools/fake-openai/scenarios/index.mjs` (E06-09 (#93)'s schema-driven matcher): schema name `weekly_review` → `tools/fake-openai/scenarios/weekly-review.json`:
```json
...
4. **Today next Monday shows the commitments**: if the test-clock helper exists by then (E11-06 (#121) plans `POST /api/testing/clock` for the comeback flow), set it to next Monday 08:00 in the user's timezone, `goto('/')` → NBA title "Morning focus block"; otherwise (helper absent) assert the same through `GET /api/today?date=<nextMonday>` if E05-01 (#38) accepts a date override, else through `/path` → WORK outcome → **Upcoming commitments** listing four "Morning focus block" rows next week — and leave a `test.info().annotations` note naming which path ran.
5. **AI down → template review still renders**: new user, `seedMixedWeek`; admin fixture sets `PUT /api/ai-settings {baseUrl: 'http://fake-openai:1/v1'}` (E05-07 (#55)'s pattern; restored in `afterEach`); `goto('/progress/week')` → **Generate review** → tiles render, notice "coach was unavailable", no `review-proposal`, "Not yet" list may be empty.
...
- `CLAUDE.md`: endpoints blocks, tables, env vars (E10-02 (#73)/03 added them — verify), a "Weekly loop" pointer paragraph to the spec under Common Patterns (do not restate rules).
...
**UI (frontend-dev)** — only `data-testid`s missing from E10-04 (#84)'s list, if any.
...
- The test-clock helper itself (E11-06 (#121)); CI workflow files (declined project-wide; local runs only).
- Visual baselines for the review page (E10-04 (#84) owns them).
- Cron timing e2e (covered by the E10-02 (#73) unit test with a fake clock; the epic script's step 14 is the manual check).

#### Notes for the implementing agent

- Reuse `tests/e2e/helpers/auth.helper.ts` (`withAiKey`, `withOnboarding`), `seed.helper.ts` (E06-09 (#93)) and `commitments.helper.ts` (E05-07 (#55)); do not create a second login or seed helper — extend them.
...
- The reviewer input is the API's, not the browser's: scenario selection must key on `text.format.name === 'weekly_review'` exactly as E06-09 (#93) does for `coach_reply`.
