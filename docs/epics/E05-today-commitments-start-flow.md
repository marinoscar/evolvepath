...
8. On the HEALTH card open the ⋯ menu → **Reschedule** → pick today 20:00 → save. The card now lists a **new** row at 20:00 with a `1` reschedule badge (the original is closed). On that row repeat, picking tomorrow 07:00. `evopath api GET /api/commitments/<healthId> | jq '.data | {status, rescheduleCount, rescheduledToId}'` → `RESCHEDULED`, `0`, `<id2>`; follow `rescheduledToId` twice → the live row: `status PLANNED`, `rescheduleCount 2`, `rescheduledFromId` set; the card no longer shows it today.
...
- [ ] Rescheduling a PLANNED commitment returns a **new** PLANNED card with `rescheduleCount: 1` and `rescheduledFromId` = the original id, and the original's status is `RESCHEDULED`; rescheduling the returned row again yields a live row with `rescheduleCount: 2`, both moved-from rows `RESCHEDULED`, and two `rescheduled` evidence rows with `from`/`to`/`count` — one on each row the reschedules created, none on the original.
...
4. `…/actions/reschedule --data '{"scheduledStart":"<tomorrow 07:00 ISO>"}'` on `<healthId>` → 200 with a **new** id (`status: PLANNED`, `rescheduleCount: 1`, `rescheduledFromId: <healthId>`); repeat on that new id → `rescheduleCount: 2`. `evopath api GET /api/commitments/<healthId> | jq '.data | {status, rescheduledToId}'` → `RESCHEDULED`; `start` on `<healthId>` → 409 `INVALID_TRANSITION` (terminal); `start` on the live id works.
...
- [ ] Case 2 proves, through the public API after two UI reschedules, that both moved-from rows are `RESCHEDULED` and the live row has `rescheduleCount === 2` and `rescheduledFromId` set.
