...
- Builds on E01: `AiGatewayService.invoke({persona:'planner', …, schema, schemaName})` (`apps/api/src/ai/gateway/ai-gateway.service.ts`) returning `{ok:true, output}` / `{ok:false, error}`; `RequireAiKey` (`apps/web/src/components/common/RequireAiKey.tsx`) and the `/setup/ai-key` full-screen page outside `Layout`; `GET /api/auth/me` carrying `aiKey:{configured,hint}` (E01-05) — `onboarding:{completed}` follows the same pattern; the fake OpenAI server `tools/fake-openai/server.mjs` + `infra/compose/fake-openai.compose.yml` (E01-10) for e2e.
- Builds on E02-01's domain schema (`apps/api/prisma/schema.prisma`): `BestSelfProfile`, `Outcome` (domain enum `WORK|FAMILY|HEALTH`), `Plan` + `PlanVersion` (`status`, `rationale`, `userApproved`, `createdBy USER|AI`), `Routine` (trigger type/value, frequency, estimated/minimum minutes, `fallbackBehavior`), `Commitment` (status enum starting at `PLANNED`, full/short/minimum versions), `DomainMode`. E02-05's shell already routes `/` to the Today placeholder.
...
- Spec file produced by this epic: `docs/specs/onboarding.md` (E04-06).

### Scope
- [ ] E04-01 feat(db): add user_profiles with onboarding state and expose onboarding status on /auth/me
- [ ] E04-02 feat(api): add onboarding endpoints with AI plan proposal, confidence check, approve and skip-ai templates
- [ ] E04-03 feat(web): add the onboarding wizard at /onboarding with per-step persistence
- [ ] E04-04 feat(web): add the first-Path proposal review screen with inline edits and confidence check
- [ ] E04-05 feat(web): gate the app shell behind onboarding completion after the AI key gate
- [ ] E04-06 test(tests): E04 end-to-end verification
...
### Sequencing
- E04-01 first (schema + `/auth/me`). E04-02 depends on E04-01 and on E01-06 (gateway) and E02-01/02/03/04 (domain tables + services to persist into).
- E04-03 and E04-05 can start against MSW as soon as E04-01's `User.onboarding` type is agreed; E04-04 depends on E04-02's proposal contract and E04-03's wizard shell.
- Critical path: E04-01 → E04-02 → E04-04 → E04-06. E04-03 and E04-05 run in parallel with E04-02.
- E04-06 last; it needs the fake OpenAI server from E01-10 and the Today placeholder from E02-05.
...
3. Open http://localhost:3535/testing/login. Email `newbie@test.local`, role `viewer`, leave "Seed OpenAI key" (E01-10) checked and untick "Mark onboarding complete" (E04-06). Submit.
...
8. Step 9: the value-exchange text, then `Allow notifications` → browser prompt → grant or dismiss; `Finish` either way. You land on http://localhost:3535/ (Today placeholder from E02-05) showing the three commitments.
...
model UserProfile {
  ...
  selectedDomains       Domain[]        @default([]) @map("selected_domains") // E02-01's enum
...
Add `profile UserProfile? @relation("UserProfile")` to `model User`. `Domain` is the enum E02-01 introduced for `Outcome.domain` — reuse it, do not declare a second one. Migration: `npm run prisma:migrate:dev -- --name add_user_profiles`. Seed: none (rows are created lazily; `prisma/seed.ts` untouched).
...
- `pendingProposal` is validated by E04-02's `onboardingProposalSchema`; this issue only declares the column.
...
- `update(userId, patch)` — typed partial used by E04-02.
...
**UI (frontend-dev)** — `apps/web/src/types/index.ts`: `User.onboarding: { completed: boolean }` **required** (a fixture that omits it fails to compile, which is the point). `apps/web/src/__tests__/mocks/data.ts` `mockUsers` and `apps/web/visual/main.tsx`'s fake user gain `onboarding: { completed: true }`. Nothing renders it yet (E04-05).
...
#### Out of scope
- Onboarding endpoints (E04-02), any UI (E04-03..05)
- Quiet-hours editing UI (E12-05); `quietHoursStart/End` are declared now so E12 does not migrate twice
- Timezone auto-detection endpoint — set by `POST /onboarding/start` in E04-02
...
- `Domain[]` with `@default([])` requires the `Domain` enum from E02-01 to exist; if E02-01 named it differently (e.g. `LifeDomain`), use that name and say so in the PR — never introduce a parallel enum.
- Run `npm run prisma:generate` after the schema change; use `npm run prisma:migrate:dev`, never bare `npx prisma`.
- Mirror the `aiKey` wiring in `AuthService.getCurrentUser` (E01-05) — same shape, same DTO style, same spec cases.
...
**Part of epic:** E04 · **Blocked by:** E04-01, E01-06, E02-02, E02-03, E02-04 · **Component:** api · **Priority:** P0 · **Agents:** backend-dev → testing-dev → docs-dev
...
**Data (database-dev)** — n/a (columns from E04-01; writes into E02-01's tables).

**API (backend-dev)** — all routes plain `@Auth()`; the profile row is the caller's own (`userId` from `@CurrentUser('id')`). OpenAPI tag `Onboarding` added to `apps/api/src/openapi/tags.ts` in the product group E02-02 introduced for Best Self / Outcomes (place it directly after `Best Self`).
...
**UI (frontend-dev)** — n/a here; `services/api.ts` functions are added by E04-03 (`getOnboardingState`, `startOnboarding`, `patchOnboardingAnswers`, `proposeOnboarding`, `submitOnboardingConfidence`, `approveOnboarding`, `skipOnboardingAi`).
...
**Docs (docs-dev)** — `docs/API.md` new "Onboarding" section (all 7 routes, error codes); `CLAUDE.md` "API Endpoints" list; `docs/specs/onboarding.md` is written by E04-06 — leave a TODO pointer in the PR, not a partial file.
...
#### Out of scope
- Any UI (E04-03/04), the route gate (E04-05), e2e (E04-06)
- Editing outcomes/plans after approval (E02-06 Path screen, E06 proposals)
- Notification preferences — step 9 is client-only (browser permission) in this epic

#### Notes for the implementing agent
- Gateway contract and error codes: `apps/api/src/ai/gateway/ai-gateway.types.ts`, `ai-errors.ts` (E01-06). Never call the provider directly and never catch-and-rethrow gateway results as 500.
- Persist through E02's services where they exist (`apps/api/src/best-self/`, `apps/api/src/outcomes/`, `apps/api/src/plans/`, `apps/api/src/commitments/` per E02-02..04) if they accept a `tx` client; otherwise use `prisma` directly inside the `$transaction` — do not nest a service's own `$transaction` inside yours.
- Field names on `PlanVersion`/`Routine`/`Commitment` are E02-01's; check `schema.prisma` before writing the create payloads rather than trusting this text.
...
**Part of epic:** E04 · **Blocked by:** E04-01, E04-02 · **Component:** web · **Priority:** P0 · **Agents:** frontend-dev → testing-dev → docs-dev
...
| 8 | `PROPOSAL` | `ProposalStep` — **E04-04** | proposal review | E04-04 |
...
**Docs (docs-dev)** — `docs/specs/onboarding.md` (E04-06 writes it; this issue adds its screen table to the PR description).
...
#### Manual test script
1. Epic script steps 1–6 (steps 7–8 need E04-04; until then, step 8 shows the "proposal coming in E04-04" placeholder).
...
#### Out of scope
- The proposal screen (E04-04) and the route gate (E04-05)
- Voice input, images/attachments in onboarding
- Quiet-hours selection (E12-05)

#### Notes for the implementing agent
- Full-screen page pattern: `apps/web/src/pages/AiKeySetupPage.tsx` (E01-08/09) and `apps/web/src/pages/ActivateDevicePage.tsx`.
...
- Pitfall: `AuthContext.refreshUser()` must run before `navigate('/')` on `Finish`, or E04-05's gate bounces the user straight back to `/onboarding`.
...
**Part of epic:** E04 · **Blocked by:** E04-02, E04-03 · **Component:** web · **Priority:** P0 · **Agents:** frontend-dev → testing-dev → docs-dev
...
**Docs (docs-dev)** — `docs/specs/onboarding.md` (E04-06) describes the confidence loop; add the copy strings to the PR so docs-dev can quote them.
...
#### Out of scope
- Editing outcomes and routines before approval (Path screen E02-06 and E06 proposals handle later edits)
- Diff rendering between proposals (E06-07's proposal cards)
...
- Keep copy strings in `apps/web/src/components/onboarding/copy.ts` (new) so E04-06's e2e spec and docs quote one source.
- Pitfall: after `approve` succeeds, do not call `refreshUser()` yet — step 9 still needs to render inside the wizard; E04-03's `Finish` handler refreshes and navigates.

---

### E04-05 `feat(web): gate the app shell behind onboarding completion after the AI key gate`

**Part of epic:** E04 · **Blocked by:** E04-01, E04-03, E01-09 · **Component:** web · **Priority:** P0 · **Agents:** frontend-dev → testing-dev → docs-dev

#### Problem statement
PRD §102 lists "see Today screen" as the last onboarding outcome, and PRD §101 (Day 0) has the user arrive at Today only after their first Path exists. Today with no commitments is an empty screen and Path with no Best Self is an empty tree. The route tree in `apps/web/src/App.tsx` gates on authentication (`ProtectedRoute`) and on the OpenAI key (`RequireAiKey`, E01-09) but nothing sends a signed-in, keyed, un-onboarded user to `/onboarding`.
...
- `apps/web/src/__tests__/pages/OnboardingPage.test.tsx`: the completed-user redirect (if not already in E04-03).

**Docs (docs-dev)** — `docs/specs/settings-ui.md` gains a short "Route gates" subsection listing the three gates and their exemptions (or `docs/specs/onboarding.md` §Gating in E04-06 with a cross-link — pick one, cross-link from the other); `CLAUDE.md` "Architecture Principles" gets one line: "Route gates: `ProtectedRoute → RequireAiKey → RequireOnboarding → Layout`; exemptions `/activate`, `/setup/ai-key`, `/onboarding`."
...
3. Complete onboarding (E04-04) → `/`; type `/onboarding` → `/`.
...
#### Notes for the implementing agent
- Copy `apps/web/src/components/common/RequireAiKey.tsx` (E01-09) and its test file; the shape and the `state={{ from }}` convention are identical.
...
5. `completes without AI via the template path` — route `**/v1/responses` through `page.route` to return 503 (or set `x-fake-behaviour` via the E01-10 mechanism) → step 8 shows `Continue without AI` → template renders → approve → `/` with 3 commitments; API `/api/plans/...` shows `createdBy: 'USER'`.
...
#### Notes for the implementing agent
- Follow `tests/e2e/specs/ai-key-gate.spec.ts` (E01-10) for the fake-server setup, `withAiKey` plumbing through `TestLoginPage` and `auth.helper.ts`, and the API-check style with `page.request`.
