<#
.SYNOPSIS
    Evolve Path Development Script for Windows

.DESCRIPTION
    Manages the Evolve Path development environment using Docker Compose.
...
if (-not (Test-Path $BaseCompose)) {
    ...
    Write-Err "Make sure you're running this script from the Evolve Path repository."
    exit 1
}

function Show-Help {
    Write-Host ""
    Write-Info "Evolve Path Development Script"
    Write-Host "====================================="
    Write-Host ""
...
function Start-Services {
    Write-Info "Starting Evolve Path services..."
    if ($Otel) {
...
function Stop-Services {
    Write-Info "Stopping Evolve Path services..."
    if ($Service) {
...
function Restart-Services {
    Write-Info "Restarting Evolve Path services..."
    if ($Service) {
...
function Rebuild-Services {
    Write-Info "Rebuilding Evolve Path services (no cache)..."
    if ($Otel) {
        Write-Info "Including OpenTelemetry observability stack..."
    }
    if ($Service) {
...
function Clean-Services {
    ...
    if ($confirmation -eq "yes") {
        Write-Info "Cleaning up Evolve Path services and volumes..."
...
function Run-E2ETests {
    ...
    try {
        ...
        $env:POSTGRES_DB = "evolvepath_test"
